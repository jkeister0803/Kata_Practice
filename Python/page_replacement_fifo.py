...
Page replacement happens when a requested page is not in memory (page fault) and a free page cannot be used to satisfy
...
the operating system keeps track of all the pages in memory in a queue, with the most recent arrival at the back,
...
The expected output is the status of the memory after the application of the algorithm. Note that when a page is
inserted in the memory,it stays at the same position until it is removed from the memory by a page fault.
...
So, at the end we have the list [4, 5, 3], which is what you have to return. If not all the slots in the memory get
occupied after applying the algorithm, fill the remaining slots (at the end of the list) with -1 to represent emptiness
...
def fifo(n, reference_list):
    ...
    while len(memory) <= n:
        for item in reference_list:
            if len(memory) < n and item not in memory:
                read_in.append(item)
                memory.append(item)
            elif len(memory) <= n and item in memory:
                read_in.append(item)
                continue
            elif len(memory) == n and item not in memory:
                read_in.append(item)
                memory.insert(read_in[0], item)
                read_in.pop(0)
            else:
                memory.append(-1)
                continue
